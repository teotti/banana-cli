...
describe("BananaSplit CLI", () => {
  ...
  it("filters groups and friends with q, alongside the other options", async () => {
    ...
    expect(
      await runCli(
        ["friends", "--search", "ana", "--filter", "guests", "--json"],
        runtime,
      ),
    ).toBe(0);
    expect(calls[1].url.pathname).toBe("/base/friends");
  ...
  it("sweeps a wide page when the API search and a username disagree", async () => {
    ...
    // The name is asked for first, then the listing is swept.
    expect(
      calls
        .filter(({ url }) => url.pathname === "/base/friends")
        .map(({ url }) => url.searchParams.get("q")),
