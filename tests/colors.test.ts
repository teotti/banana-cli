...
describe("palette", () => {
  ...
  it("stays legible on a white and on a near-black background", () => {
  ...
  it("keeps the wordmark's gradient visible either way", () => {
    for (const shade of SHADES) {
      expect(contrast(shade, WHITE)).toBeGreaterThanOrEqual(2.5);
      expect(contrast(shade, NEAR_BLACK)).toBeGreaterThanOrEqual(2.5);
    }
  });
});
